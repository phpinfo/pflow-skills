...
| [`pflow-glossary`](skills/pflow-glossary) | Creates or extends `GLOSSARY.md` — one table of canonical terms, their code identifiers, one-sentence definitions and banned synonyms — so docs, code and conversation use one name per concept. Audits docs and identifier frequencies for synonym clusters, asks which name is canonical, lint-checks and sorts the table, and adds a one-line "use the glossary" rule to `AGENTS.md`/`CLAUDE.md`. Invoked manually. |
