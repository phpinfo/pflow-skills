...
1. Pick the context source:
   ...
   - Otherwise (changes made outside this session, unknown working-tree state, or any doubt) run `.agents/skills/pflow-commit/scripts/git-commit-context.sh`. If the output is `No changes detected.`, tell the user there is nothing to commit and stop.
2. Compose MESSAGE (see format below) using ONLY the step-1 source — no other sources.
...
- **The step-1 source is the ONLY input.** Do not enrich or verify it: no reading files, no `git` commands, no grep/search, no sub-agents. When the script was run, ignore conversation history too. If the context is incomplete or truncated, write the message from what it does contain — describe only what is visible and stay generic about the rest. Never ask the user for more context.
- Skipping the script is a bet that the working tree holds nothing but this session's changes. `git add -A` will commit whatever else is there under your message — if you are not certain, run the script.
- The script output is truncated: at most 50 lines per file and 600 lines total. Large diffs are shown only partially — don't draw conclusions about the cut-off part.
