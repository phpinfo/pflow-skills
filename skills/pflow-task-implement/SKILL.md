...
2. **Review.** Read `plan_file` critically. If a step is ambiguous, contradictory, or has a blocking gap, stop and ask — never guess.
...
4. **Verify.** Run the check the plan specifies for each step; if none, confirm by reading back the change. On any blocker, stop and ask rather than work around it.
